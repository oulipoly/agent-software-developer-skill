...
class ResearchBranchBuilder:
    """Translates semantic tickets into concrete BranchSpec objects."""
    ...
    def build_branch(
        ...
        *,
        ...
        if research_type == RESEARCH_TYPE_WEB:
            return self._build_web_branch(*args)

        if research_type == RESEARCH_TYPE_CODE:
            return self._build_code_branch(*args)
    ...
    def _build_both_branch(
        ...
        web_ticket["output_path"] = str(
            PathRegistry(planspace).research_ticket_result(
                section_number, ticket_index, RESEARCH_TYPE_WEB,
            )
        )
