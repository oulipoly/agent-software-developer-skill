...
class CompletionHandler:
    """Post-section-completion orchestration."""
    ...
    def post_section_completion(
        ...
        snapshot_dir = snapshot_modified_files(
            ...
            warn=lambda msg: self._logger.log(f"Section {sec_num}: WARNING \u2014 {msg}"),
        )
        self._logger.log(f"Section {sec_num}: snapshotted {len(modified_files)} files to {snapshot_dir}")
        ...
        for target_num, reason, _contract_risk, note_md in impacted_sections:
            ...
            self._logger.log(f"Section {sec_num}: left note for section {target_num} at {note_path}")
        ...
        self._write_contract_artifacts(
    ...
    def read_incoming_notes(
        ...
        if ack_path.exists():
            ack_data = self._artifact_io.read_json(ack_path)
            if isinstance(ack_data, dict):
                ...
                self._artifact_io.rename_malformed(ack_path)
                self._logger.log(
