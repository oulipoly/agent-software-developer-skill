...
def _parse_coordination_plan(
    ...
    return _get_planner()._parse_coordination_plan(agent_output, problems)


def write_coordination_plan_prompt(
    problems: list[Problem], planspace: Path,
) -> Path:
    return _get_planner().write_coordination_plan_prompt(problems, planspace)


def _normalize_bridge_directives(plan: dict[str, Any]) -> None:
    _get_planner()._normalize_bridge_directives(plan)


__all__ = [
    "_parse_coordination_plan",
    "_normalize_bridge_directives",
