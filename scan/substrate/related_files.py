...
from scan.related.cli_handler import block_insert_position, extract_related_files
...
class RelatedFiles:
    ...
    def _read_signal_failclosed(self, path: Path) -> dict | None:
    ...
    def apply_related_files_updates(self, planspace: Path) -> int:
        ...
        for signal_path in sorted(signals_dir.glob("section-*.json")):
            ...
            data = self._read_signal_failclosed(signal_path)
            if data is None:
                continue
