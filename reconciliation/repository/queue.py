...
class Queue:
    """Handles persistence of reconciliation request artifacts."""
    ...
    def queue_reconciliation_request(
        ...
        self._artifact_io.write_json(request_path, request)
        logger.info(
    ...
    def load_reconciliation_requests(self, planspace: Path) -> list[dict]:
        ...
        for req_path in sorted(recon_dir.glob("section-*-reconciliation.json")):
            data = self._artifact_io.read_json(req_path)
            if data is None:
                logger.warning(
