...
class ToolSurfaceWriter:
    """Reads registry, repairs if needed, and writes tool surfaces."""
    ...
    def surface_tool_registry(
        ...
        *,
        ...
        if registry is not None:
            ...
            self._log_surface_result(
                section_number, relevant_count, len(all_tools), tools_available_path,
            )
            return len(all_tools)

        dispatched = self._dispatch_registry_repair(
            section_number=section_number,
            planspace=planspace,
            codespace=codespace,
        )
        if not dispatched:
            return 0
        return self._handle_repair_result(
            section_number=section_number,
            planspace=planspace,
        )
